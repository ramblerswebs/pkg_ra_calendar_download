 ...
 */
class ra_calendar_download_feed
{
	/**
	...
	public function update_records_url($url)
	{
			...
			if (JDEBUG) { JLog::add("Commencing download of groups using url " . $url, JLog::DEBUG, "pkg_ra_calendar_download"); }
            $ra_feed = new RJsongroupsFeed($url);
            ...
            foreach ($groups as $group)
            {
                ...
                {
					// Now check to see if the entry already exists within the system.
					if (!$this->group_exists($group->groupCode))
					{					
						if (JDEBUG) { JLog::add("Group not found, adding " . $group->groupCode, JLog::DEBUG, "pkg_ra_calendar_download"); }
	...
	private function group_exists($group)
	{
		...
		$query = $db->getQuery(true);
		$query->select('COUNT(*)');
		$query->from($db->quoteName('#__ra_groups'));
		$query->where($db->quoteName('code')." = ".$db->quote($group));

		// Reset the query using our newly populated query object.
		$db->setQuery($query);
		$count = $db->loadResult();
		if ($count > 0) $return_value = true ;
