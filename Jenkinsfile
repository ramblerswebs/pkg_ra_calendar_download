pipeline {
  ...
  parameters {
        choice(
            choices: ['Yes' , 'No'],
            description: 'Deploy Succcessful Build to Local Apache01 Site',
            name: 'DEPLOY_APACHE01_SITE')

        choice(
            choices: [‘No’ , ‘Yes’],
            description: ‘Deploy Succcessful Build to Local Apache02 Site’,
            name: ‘DEPLOY_APACHE02_SITE’)

        choice(
            choices: [‘No’ , ‘Yes’],
            description: ‘Deploy Succcessful Build to Local Apache03 Site’,
  ...
  stages {
    ...
    stage('Update version information') {
      steps {
        sh 'python /usr/python/UpdateJoomlaBuild -bx -i pkg_ra_calendar_download/packages/com_ra_calendar_download/com_ra_calendar_download.xml'
        sh 'python /usr/python/UpdateJoomlaBuild -bx -i pkg_ra_calendar_download/packages/mod_ra_calendar_download/mod_ra_calendar_download.xml'
